...
from nemo.collections.asr.data.text_to_text_clip import (
...
from nemo.collections.tts.models import FastPitchModel, SpectrogramEnhancerModelClip
...
class ASRWithTTSModelClip(ASRModel):
    """
    ...
    enhancer_model: Optional[SpectrogramEnhancerModelClip]
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
       ...
       # tts model
        ...
        if cfg.enhancer_model is not None:
            print("cfg.enhancer_model is not None")
            self.register_nemo_submodule(
                "enhancer_model", config_field="enhancer_model", model=SpectrogramEnhancerModelClip(cfg.enhancer_model)
            )
        elif cfg.enhancer_model_path is not None:
            print("cfg.enhancer_model_path is not None", cfg.enhancer_model_path)
            if cfg.enhancer_model_path.split("/")[-1].split(".")[-1] == "nemo":
            # 如果是.nemo文件，使用FastPitchModel恢复
                model = SpectrogramEnhancerModelClip.restore_from(cfg.enhancer_model_path, map_location=torch.device("cpu"))
            elif cfg.enhancer_model_path.split("/")[-1].split(".")[-1] == "ckpt":
            # 如果是.ckpt文件，使用FastPitchModel从检查点加载
                model = SpectrogramEnhancerModelClip.load_from_checkpoint(cfg.enhancer_model_path, map_location=torch.device("cpu"))
            self.register_nemo_submodule(
                "enhancer_model",
                config_field="enhancer_model",
                model=model #SpectrogramEnhancerModel.restore_from(cfg.enhancer_model_path, map_location=torch.device("cpu")),
            )
        else:
            print("enhancer is None")
    ...
    def from_pretrained_models(
        ...
        return ASRWithTTSModelClip(cfg, trainer=trainer)
    ...
    def _get_tts_spectrogram(
        self, tts_texts: torch.Tensor, text: List[str], speakers: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:  # _embedding
        ...
        with torch.no_grad():
            ...
            if self.enhancer_model is not None:
                ...
                with typecheck.disable_checks():
                    ...
                    spectrogram = self.enhancer_model.forward(input_spectrograms=spectrogram, text=text, lengths=spectrogram_len)
    ...
    def _get_batch_spect(self, batch: Union[TextToTextBatch, TextOrAudioToTextBatch, tuple]):
        ...
        if isinstance(batch, TextToTextBatch):
            # print('isinstance(batch, TextToTextBatch)', batch.tts_texts.dtype)
            spectrogram, spectrogram_len = self._get_tts_spectrogram(batch.tts_texts, batch.text, batch.speakers)
            transcript = batch.transcripts
            transcript_len = batch.transcript_lengths
        elif isinstance(batch, TextOrAudioToTextBatch):
            tts_spectrogram, tts_spectrogram_len = self._get_tts_spectrogram(batch.tts_texts, batch.text, batch.speakers)  #  
            # tts_text 已经是 token ，text 是字符串数组
